...
export function createMelodyBricksEngine(audioAdapter) {
    const melodyBricks = {
        intervalLabelVector: [
        ...
        // Use C-short as the default positive feedback sound
        correctSound: 'c-short',
        correctVolume: 0.5,
        incorrectSound: 'negative',
        incorrectVolume: 0.5,

        // Helper function to get sound file path based on sound type
        getSoundFilePath(soundType, isCorrect) {
            if (soundType === 'off') return null;
            
            const soundMappings = {
                correct: {
                    'retrowin': '/sound/game_sounds/RetroWin.wav',
                    'c-short': '/sound/game_sounds/C-short.wav',
                    'c-bell': '/sound/game_sounds/C-bell.wav',
                    'c-voice': '/sound/game_sounds/C-voice.wav'
                },
                incorrect: {
                    'negative': '/sound/game_sounds/Negative.wav',
                    'w-short': '/sound/game_sounds/W-short.wav',
                    'w-down': '/sound/game_sounds/W-down.wav',
                    'w-voice': '/sound/game_sounds/W-voice.wav'
                }
            };
            
            const category = isCorrect ? 'correct' : 'incorrect';
            return soundMappings[category][soundType] || null;
        },

        startTraining(
            intervalScoreVector,        // 2D array of interval scores (36 intervals × 10 scores each) - USED for tracking progress and winning condition
            task,                       // Main game task type (e.g., 'notes', 'intervals') - USED to determine game mode
            subTask,                    // Sub-task type (e.g., 'notes', 'intervals') - USED for specific game variant
            currentLevel,               // Level object containing settings and win conditions - USED for level configuration and brick clearing
            randomIntervalRoot,         // Boolean to randomize root note for intervals - USED to determine if root note changes during gameplay
            whichKeys,                  // Keyboard display mode ('onScreen' or other) - USED for UI configuration
            oct,                        // Octave setting for note generation - USED for note list generation
            level,                      // Level number/identifier - USED for level progression tracking
            rangeLoLimit,               // Lower limit for note range - USED for note generation boundaries
            rangeUpLimit,               // Upper limit for note range - USED for note generation boundaries
            tonality,                   // Musical key (e.g., 'C', 'F#') - USED for audio playback and display
            trainingDurationArg,        // Training session duration in milliseconds - USED for timer system (defaults to 5 minutes)
            refNumArg,                  // Reference number for audio playback - USED for reference sounds
            audioCom,                   // Audio feedback enabled/disabled - USED for sound effects
            visualCom,                  // Visual feedback enabled/disabled - USED for UI elements
            replaytAfter,               // Delay before replaying interval (milliseconds) - USED for timing
            showAnimation,              // Flag for animation display - USED for visual effects
            isLoggedIn = true,          // User authentication status - USED for data management (currently commented out)
            existingKeyRangeList = null, // Pre-existing key range list for 12-tone wall levels - USED to avoid regenerating keys
            correctSound = 'retrowin',  // Sound for correct answers
            correctVolume = 0.5,        // Volume for correct answer sounds
            incorrectSound = 'negative', // Sound for incorrect answers
            incorrectVolume = 0.5       // Volume for incorrect answer sounds
        ) {

            console.log('🚀 GAMEENGINE: startTraining called');
            console.log('🚀 GAMEENGINE: trainingStarted before =', this.trainingStarted);
            ...
            console.log('🚀 GAMEENGINE: trainingStarted set to true');
            this.showAnimation = showAnimation;
            this.rowsCleared = 0;
            this.bonuses = 0;
            
            // Store feedback sound settings
            this.correctSound = correctSound;
            this.correctVolume = correctVolume;
            this.incorrectSound = incorrectSound;
            this.incorrectVolume = incorrectVolume;
            ...
            // Replace document.dispatchEvent with eventEmitter.emit
 ...
 */
        ...
        handleTimeUp() {
            console.log('⏰ GAMEENGINE: handleTimeUp called - but timer no longer ends the game');
            // Timer no longer ends the game - game ends when bricks reach top row
            // Just update the display, don't stop training
            this.monitorMessage = "Time's up, but keep playing! Game ends when bricks reach the top.";
            eventEmitter.emit('updateMonitorMessage', this.monitorMessage);
            
            // Don't emit updateControlEvent or stop training
            // The game continues until bricks reach the top row
        },
        ...
        */
        ...
        stopTraining() {
            console.log('🛑 GAMEENGINE: stopTraining called, setting trainingStarted to false');
            this.trainingStarted = false;
            console.log('🛑 GAMEENGINE: trainingStarted set to false in stopTraining');
            // this.monitorMessage = "";
        ...
        playReference() {
            ...
            audioAdapter.playAudio(relativeFile);
        ...
        */
        ...
        checkWinningCondition() {
            ...
            console.log('🔍 intervalScoreVector:', this.intervalScoreVector);
            console.log('🔍 descendingVector:', this.descendingVector);
            console.log('🔍 ascendingVector:', this.ascendingVector);
            console.log('🔍 harmonicVector:', this.harmonicVector);
            let vectorStart = 0;
            ...
            console.log('🔍 WINNING CONDITION: Need 5 scores of "1" for each interval marked as "1" in intervalVector');
            // console.log("intervalVector: " + this.intervalVector);
            for (let i = 0; i < this.intervalVector.length; i++) {
                // console.log("intervalVector[" + i + "]: " + this.intervalVector[i]);
                if (this.intervalVector[i] === 1) {
                    console.log('🔍 Interval', i, 'is selected (marked as 1), checking score array at index', i + vectorStart);
                    const scoreArray = this.intervalScoreVector[i + vectorStart];
                    console.log('🔍 Score array at index', i + vectorStart, ':', scoreArray);
                    const scoreCount = scoreArray ? scoreArray.filter(val => val === 1).length : 0;
                    console.log('🔍 Score count for interval', i, ':', scoreCount, '/5 needed');
                    // Validate that the scoreArray exists and has exactly five 1s
                    if (!Array.isArray(scoreArray) || scoreCount !== 5) {
                        console.log('🔍 Winning condition not met for interval', i, '- array length or score count insufficient');
                        return false; // Winning condition not met
                    }
                    console.log('🔍 Interval', i, 'has 5 scores - winning condition met for this interval');
                }
            }
            console.log('🔍 All selected intervals have 5 scores - WINNING CONDITION MET!');
            return true; // All required intervals have 5 scores
        },

        repeatInterval() {
            eventEmitter.emit('updateMonitorMessage', "Root note is " + this.tonality + "4");

            let intervalDelay = 0;
            if (this.selectedInterval === 'harmonic') {
                const intervalFile = "/sound/piano_wav/" + getFileName(this.randomMidiNumber);
                const harmonicFiles = [this.rootFile, intervalFile];
                audioAdapter.playAudioChords(harmonicFiles);
            }
            else {
                ...
                if (this.subTask === 'intervals') {
                    ...
                    audioAdapter.playAudio(this.rootFile);
                ...
                setTimeout(() => {
                    ...
                    audioAdapter.playAudio(this.fileName);
                }, intervalDelay);
        ...
        runNewNoteCycle() {
            ...
            eventEmitter.emit('updateMonitorMessage', "Target #" + this.notesGenerated);
        ...
        handleWrongNote() {
            if (this.notReceivedPointsYet) {
                ...
                eventEmitter.emit('updateMonitorMessage', "Incorrect !");
                ...
                if (this.audioCom) {
                    const soundFile = this.getSoundFilePath(this.incorrectSound, false);
                    if (soundFile) {
                        if (this.whichKeys === 'onScreen') {
                            setTimeout(() => {
                                audioAdapter.playAudio(soundFile, this.incorrectVolume);
                            }, 500);
                        }
                        else {
                            audioAdapter.playAudio(soundFile, this.incorrectVolume);
        ...
        handleCorrectNote() {
            if (this.notReceivedPointsYet) {
                ...
                eventEmitter.emit('updateMonitorMessage', "Correct !");
                ...
                this.notReceivedPointsYet = false;
                if (this.audioCom) {
                    const soundFile = this.getSoundFilePath(this.correctSound, true);
                    if (soundFile) {
                        if (this.whichKeys === 'onScreen') {
                            setTimeout(() => {
                                audioAdapter.playAudio(soundFile, this.correctVolume);
                            }, 500);
                        }
                        else {
                            audioAdapter.playAudio(soundFile, this.correctVolume);
                ...
                console.log('🎯 GAMEENGINE: About to check winning condition after correct note');
                console.log('🎯 GAMEENGINE: Current intervalIndex:', this.intervalIndex);
                console.log('🎯 GAMEENGINE: Current correctAnswers:', this.correctAnswers);
                console.log('🎯 GAMEENGINE: Current notesGenerated:', this.notesGenerated);
                
                // Removed old checkWinningCondition() logic - winning condition is now based on rows cleared
                // The game ends when the required number of rows are cleared, not when interval scores reach 5
        ...
        handleGameOver() {
            setTimeout(() => {
                console.log('💀 GAMEENGINE: handleGameOver called, setting trainingStarted to false');
                eventEmitter.emit('updateMonitorMessage', "GAME OVER")
                this.trainingStarted = false;
                console.log('💀 GAMEENGINE: trainingStarted set to false in handleGameOver');
                ...
                if (this.audioCom) audioAdapter.playAudio('/game_sounds/Landing.wav');
            }, 1000);
        },
        ...
        handleBrickLanded(data) {
            ...
            console.log('🎮 GAMEENGINE: Received brickLanded event:', data);
            console.log('🎮 GAMEENGINE: gameOver =', this.gameOver, 'trainingStarted =', this.trainingStarted);
            console.log('🎮 GAMEENGINE: intervalActive =', this.intervalActive);
            console.log('🎮 GAMEENGINE: currentLevel =', this.currentLevel?.id);
            this.intervalActive = false;
            console.log('🎮 GAMEENGINE: About to check conditions: gameOver =', this.gameOver, 'trainingStarted =', this.trainingStarted);
            if (!this.gameOver && this.trainingStarted) {
                console.log('🎮 GAMEENGINE: Conditions met, processing brick landing...');
                // Check if it landed in row 0 (top row) - game over condition
                if (data.isGameOver) {
                    console.log('🎮 GAMEENGINE: Game over condition triggered');
                    console.log('🎮 GAMEENGINE: Setting trainingStarted to false due to isGameOver = true');
                    // Game over - brick reached the top
                    this.gameOver = true;
                    this.trainingStarted = false;
                    console.log('🎮 GAMEENGINE: trainingStarted set to false in handleBrickLanded due to isGameOver');
                    ...
                    console.log('🎮 GAMEENGINE: Normal brick landing, setting up setTimeout...');
                    this.longTime = 1000; // 1 second delay before next note
                    console.log('🎮 GAMEENGINE: longTime set to:', this.longTime, 'ms');

                    // Clear any existing timer
                    if (this.correctNoteTimer) {
                        console.log('🎮 GAMEENGINE: Clearing existing correctNoteTimer');
                        clearTimeout(this.correctNoteTimer);
                    }

                    console.log('🎮 GAMEENGINE: Setting setTimeout for', this.longTime, 'ms');
                    this.correctNoteTimer = setTimeout(() => {
                        console.log('🎵 GAMEENGINE: setTimeout callback executed - calling runNewNoteCycle');
                        this.runNewNoteCycle();
                    }, this.longTime);
                    console.log('🎮 GAMEENGINE: setTimeout set with ID:', this.correctNoteTimer);
                }
            }
        },
        ...
        handleRowsCleared(data) {
            if (data && data.rowsCleared > 0) {
                ...
                if (this.rowsCleared >= requiredRows) {
                    // Game won!
                    console.log('🏆 GAMEENGINE: Win condition met, setting trainingStarted to false');
                    this.gameOver = true;
                    this.trainingStarted = false;
                    console.log('🏆 GAMEENGINE: trainingStarted set to false in handleRowsCleared due to win');

                    eventEmitter.emit('updateMonitorMessage', `🏆 LEVEL COMPLETE! You cleared ${this.rowsCleared} rows!`);

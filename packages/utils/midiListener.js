...
// Robust MIDI listener utility - Cross-platform (Web MIDI + React Native Touch)
let midiAccess = null;
...
export const initMIDI = (callback) => {
    ...
    // React Native - use touch events instead of MIDI
    if (isReactNative) {
        console.log('🎹 React Native: MIDI listener initialized for touch events');
        return;
    }
    
    // Web platform - use Web MIDI API
    if (!isWeb) return;
...
// React Native specific: Function to handle touch-based note input
export const handleTouchNote = (note) => {
    if (isReactNative && onMidiMessageCallback) {
        onMidiMessageCallback(note);
    }
};
...
// import ShowHeatMap from '@/src/components/ShowHeatMap.js';
...
// Removed legacy CSS import; all styles are now Tailwind only.
import IntervalSelector from './IntervalSelector';
import AnalyticsPlot from '@/src/components/Analytics';
import IntervalAccuracyTable from '@/src/components/IntervalAccuracyTable';

const AudiationStudioGUIDesktop = ({
  aiActive,
  setAiActive,
  score,
  intervalScoreVector,
  ...
  useEffect(() => {
    const handleSessionEnd = (sessionData) => {
      ...
      // Show popup for all users (both logged-in and non-logged-in)
  ...
  return (
    <div className="pt-20 px-40 cockpit-container">
      ...
      {!showSettings &&
        <div className="px-7 pt-16 flex justify-center">
          <div className="flex items-center justify-center w-full max-w-6xl">
            
            {/* Timer Display - Left Side */}
            <div className="flex justify-start w-1/3">
              <div className="bg-green-100 bg-opacity-80 border-2 border-green-600 rounded-xl shadow-lg text-center p-4">
                <div className="text-lg font-semibold text-green-800">Time Remaining: <span className="text-2xl font-bold text-green-900">{timeString}</span></div>
              </div>
            </div>
      ...
      {!showSettings &&
        ...
        <div className="w-full flex justify-center items-center my-4">
          <div className="bg-yellow-100 bg-opacity-80 border-2 border-yellow-600 rounded-xl shadow-lg text-center p-4">
              ...
              {showSettings &&
                <div>
                  <div className="table-cell">
                    <div className="flex gap-2">
                      <button
                        className={`px-3 py-2 rounded-md border font-bold shadow ${showSettings ? 'bg-green-700 text-white' : 'bg-white text-black hover:bg-gray-200'}`}
                        onClick={() => changeSettings('showSettings', null)}
                      >
                        Close Settings
                      </button>
                      {/* <button
                        className={`px-3 py-2 rounded-md border font-bold shadow ${showMoreSettings === 'duration' ? 'bg-green-700 text-black' : 'bg-white text-black hover:bg-gray-200'}`}
                        onClick={() => changeSettings('showMoreSettings', 'duration')}
                      >
                        Duration
                      </button>
                      <button
                        className={`px-3 py-2 rounded-md border font-bold shadow ${showMoreSettings === 'keyboard' ? 'bg-green-700 text-black' : 'bg-white text-black hover:bg-gray-200'}`}
...
export default AudiationStudioGUIDesktop;
...
import MBLevelScorePoints from './MBLevelScorePoints';
import MBLevelScoreRows from './MBLevelScoreRows';
...
import midiLookup from '@shared/utils/midiLookup';
...
const MBGUIMobile = ({
  ...
  // Auto-scroll keyboard to the right when it becomes visible
  useEffect(() => {
    if (showKeyboard && keyboardScrollRef.current) {
      // Small delay to ensure the keyboard is fully rendered
      setTimeout(() => {
        if (keyboardScrollRef.current) {
          keyboardScrollRef.current.scrollLeft = keyboardScrollRef.current.scrollWidth;
        }
      }, 100);
    }
  }, [showKeyboard]);
  ...
  return (
    <div className="pt-2 px-2 cockpit-container">
      ...
      {/* Two Row Layout when settings are hidden */}
      {!showSettings && (
        <>
          {/* Mobile Game Layout - Score elements above canvas */}
          <div className="flex flex-col w-full gap-2 pt-1">
            {/* Top Row - Back Button, Points, Rows, Bleeps - Evenly Distributed */}
            <div className="flex justify-between items-center px-2 w-full">
              {/* Back Button */}
              {onBackToMap && (
                <button
                  className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded-lg shadow-lg transition-colors duration-200 text-sm whitespace-nowrap"
                  onClick={onBackToMap}
                >
                  ← Back
                </button>
            ...
            {/* Game Canvas and Keyboard Column */}
            <div className="flex flex-col items-center gap-0 flex-1" style={{ marginLeft: '-5px', marginTop: '-10px' }}>
              {/* Game Canvas */}
              {displayGameCanvas && (
                <div className="flex justify-center w-full">
                  <GameCanvasComponentV2 currentLevel={currentLevel} isMobile={true} />
                </div>
              )}

              {/* Keyboard directly under canvas */}
              {showKeyboard && !showGameOverPopup && currentLevel?.type === "12 tone wall" && (
                <div className="flex justify-center mt-0" style={{ marginLeft: '43px' }}>
                  <MBKeyboard keyRangeList={keyRangeList} isMobile={true} />
                </div>
              )}

              {showKeyboard && !showGameOverPopup && currentLevel?.type !== "12 tone wall" && (
                <div className="bg-yellow-100 bg-opacity-80 border-2 border-yellow-600 rounded-xl shadow-lg text-center p-2 w-full" style={{ marginLeft: '43px' }}>
          ...
          {/* Game Over Popup - positioned independently at bottom of screen */}
          {showGameOverPopup && (
            <div className="fixed bottom-0 left-0 right-0 flex justify-center z-50">
              <div className="bg-yellow-100 bg-opacity-80 border-2 border-yellow-600 rounded-xl shadow-lg text-center p-2" style={{ maxWidth: '90vw' }}>
                <GameOverPopup

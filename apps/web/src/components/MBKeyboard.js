...
import '@/src/styles/MBKeyboard.css';
...
const MBKeyboard = ({ keyRangeList, isMobile = false }) => {
    ...
    return (
        <div className={`mb-keyboard ${isMobile ? 'mb-keyboard-mobile' : ''}`}>
            {keys.map(({ note, type }) => (
                <div
                    key={note}
                    className={`mb-key ${type} ${pressedKey === note ? 'pressed' : ''} ${rootNote === note ? 'green' : ''} ${correctNote === note ? 'green' : ''} ${wrongNote === note ? 'red' : ''}`}
                    data-note={note}
